---
name: nextjs
description: Implement a feature in a Next.js (App Router) + TypeScript + Tailwind + Shadcn project following Vercel performance standards. Invoke explicitly ($nextjs / /nextjs) when implementing an approved task list in a Next.js codebase.
---
...
1. Examine the written code for redundant renders, avoidable prop drilling, missing strict types, accessibility violations, image-loading issues.
2. Run/write the **smallest** relevant test scope (Playwright/Vitest `--filter` or a single route), not the whole suite unless asked.
3. If you learned new lessons / Next.js gotchas / fixed a specific bug, update `.ai/memory/lessons.md`.
