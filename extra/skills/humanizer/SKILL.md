...
3. **Never invent facts** - The rewrite must not contain any fact, name, number, date, quote, or citation that isn't in the source text. Swapping a vague claim for a specific one is allowed only when the specific comes from the source or from the user; if a sentence needs real-world detail to work, ask for it or write the plain version without it (see "Missing Specifics" below for the write-around-or-mark protocol). Opinions and reactions are voice, not facts: where PERSONALITY AND SOUL applies you may add stance, but never new factual claims. (In fiction, invented detail is the job. This rule governs everything else.)
...
## Missing Specifics: Never Invent, Rarely Leave a Hole

Generic prose is what happens when there's no specific to reach for, and it's also what makes writing read as AI. But the fix is never to invent a number, date, name, version, or stat to make a sentence land. A fabricated specific is worse than a vague sentence, because it looks checkable and nobody checks it.

Two moves, in this order, for anything you (or the draft) can't verify:

1. **Write around it.** Most sentences don't actually need the missing fact:
   - Before: "Our engagement grew from [what was it before?] to 4x in six months."
   - After: "Engagement quadrupled in six months." *(The baseline was never load-bearing.)*
...
**How to tell a real enumeration from a rhythmic triad.** Not every 3-item list is a problem, genuine enumerations of three real things are fine. Run any 3-item list through these three tests; if it fails at least two, it's padding:

1. **Deletion test** - can you delete one item without losing information the sentence needs? If yes, it's rhythmic. ("well-formatted, uses appropriate variable names, and follows common patterns" - drop any one, the claim survives.)
2. **Reorder test** - can the items be freely reordered with no loss? If yes, it's rhythmic. Genuine enumerations usually have an order imposed by logic, chronology, or scale ("specify, plan, execute" is a sequence, it fails this test, meaning it's likely real).
3. **Substitution test** - could a near-synonym replace the third item without changing what the sentence does? If yes, that item is filler. ("reliable, safe, and maintainable" - swap in "robust," nothing changes.)

For LinkedIn/blog posts specifically: watch for every list in the same piece having exactly three items - that's the tell, even more than any single list.
...
3. Ask three questions:
   - **"What makes the below so obviously AI generated?"** Answer briefly with remaining tells.
   ...
   - **"Did the rewrite fix the tell, or just move it?"** Check specifically for: relocated negation (a "not X, it's Y" got cut but the negated idea still shows up elsewhere in the paragraph), lateral substitution (an AI-vocabulary word swapped for a near-synonym, "delve" → "explore", instead of actually rewritten), signpost laundering ("Let's dive in" deleted but an implicit announcement of structure remains), and over-flattening (fixing the slop also killed the sentence-length variance and personality, leaving something duller than the original draft).
4. Revise into a **final rewrite** that addresses all three and contains no em or en dashes (see §14).
